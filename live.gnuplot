...
# ./chroma66202 -cpt -r-1 -y1 | stdbuf -oL sed 's/^\([^ ]\+\) [^ ]\+;/\1;/' >/tmp/data
# gnuplot live.gnuplot
# sed is required to remove timezone name from timestamp as gnuplot is not able to handle the timezones

# Producing live plot on the remote host
# Sender:
# ./chroma66202 -cpt -r-1 -y1 | nc -w10 host 8886
# Listener:
# stdbuf -oL nc -l -p 8886  | stdbuf -oL sed 's/^\([^ ]\+\) [^ ]\+;/\1;/' | stdbuf -oL awk -F";" '{print $1,$2}' >/tmp/data
# gnuplot live.gnuplot
...
set autoscale 
set yrange [0:]
plot "/tmp/data" using 1:3 with linespoints
pause 1
reread
