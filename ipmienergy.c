/*
  Frontend for measuring energy consumption with IPMI power meter
...
#define MODPROBE_MODULE "modprobe ipmi_devintf"
...
void help(char *argv[])
{
  printf("IPMI power meter frontend.\n");
  ...
  printf("  -t             Test mode, just check presence of the IPMI power ");
...
int open_ipmi_power_meter()
{
  if (system(MODPROBE_MODULE))
    fprintf(stderr, "Error: modprobing module: %s\n", MODPROBE_MODULE);
  return openipmi_open(&ipmi) > 0 && !openipmi_pwr_rd(&ipmi, &pwr);
}

void close_ipmi_power_meter()
{
  if (ipmi.opened)
    openipmi_close(&ipmi);
}

void read_averaging_interval()
{
  // this is for avg, not curr
//  openipmi_pwr_rd(&ipmi, &pwr);
//  averaging_interval = pwr.samp_period;
//  return pwr.samp_period;

  // it seems there is no way how to find out averaging interval for curr
  // so use dummy value of 1000 ms
  averaging_interval = 1000;
}

int read_ipmi_power_meter()
{
  // On failure silently stuff with previous value
  openipmi_pwr_rd(&ipmi, &pwr);
...
int main(int argc, char *argv[])
{
  ...
  if (!open_ipmi_power_meter())
  {
    ...
    fprintf(stderr, "Error: unable to find IPMI power meter.\n");
    return 1;
  }
